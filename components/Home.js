...
function Home() {
  ...
  const [moviesDiscovered, setMoviesDiscovered] = useState([]);
  ...
  const movies = moviesDiscovered.map((data, i) => {
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.length > 200 ? `${data.overview.slice(0, 200)}...` : data.overview} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count}/>;
  });
